...
fn_panel_bridge_status() {
    ...
    printf '"port":%d,' "${port:-7777}"
    printf '"queryport":%d,' "${queryport:-27015}"
    printf '"rconport":%d,' "${rconport:-25575}"
    printf '"rcon_enabled":%s,' "$([ "${rcon_enabled:-false}" = "true" ] || [ "${rcon_enabled:-false}" = "1" ] && printf 'true' || printf 'false')"
