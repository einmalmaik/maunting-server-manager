...
Mit dem KI-Agenten sprechen statt tippen: Der Sprachmodus nutzt dieselbe Unterhaltung, denselben Lauf, denselben Werkzeugkatalog und dieselbe Rechteprüfung wie der Chat. Pipecat ordnet dabei nur die internen Audio- und Steuerframes im Panelprozess; Spracherkennung bleibt beim bestehenden Modellzugang, Sprachausgabe bei ElevenLabs. Das Audiosignal läuft direkt über das Panel; die Werkzeugschleife bleibt serverseitig und API-Schlüssel verlassen den Panel-Prozess nicht. Die KI erbt die Rechte des Nutzers. Änderungen erzeugen dieselbe Vorschlagskarte wie im Chat; eine Bestätigung führt die Aktion aus. **Ohne eingerichtete Zugänge oder die verpflichtende Voice-Abhängigkeit bleibt die Funktion ausgeblendet.** Einrichtung in [`docs/self-hosting.md`](docs/self-hosting.md#sprachmodus-mit-der-ki-reden).
