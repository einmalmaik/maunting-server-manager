...
Mit dem KI-Agenten reden statt tippen — derselbe Agent, dieselbe Unterhaltung, dieselben Werkzeuge, nur gesprochen. Der Ton läuft über einen WebSocket **durch das Panel** und nicht am Panel vorbei: die Werkzeugschleife und damit jede Rechteprüfung bleibt serverseitig, und der Betreiberschlüssel verlässt den Panelprozess nie. Änderungen erzeugen dieselbe Vorschlagskarte wie im Chat; die KI liest den vom Panel formulierten Satz vor und führt erst bei einem klaren Ja aus. Löschen, Backup-Restore sowie Schlüssel und Rollen bleiben per Stimme unbestätigbar. **Ohne eingerichteten Sprachzugang erscheint die Funktion gar nicht.** Voraussetzung ist ein zweiter Anbieterzugang bei OpenAI (`gpt-realtime-2.1`) — OpenRouter bietet kein Duplex. Einrichtung in [`docs/self-hosting.md`](docs/self-hosting.md#sprachmodus-mit-der-ki-reden).
