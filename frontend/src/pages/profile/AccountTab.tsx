...
import { Mail, AlertTriangle, Clock, Globe, MapPin, Save, ShieldCheck, Trash2, Camera, Loader2, Shield, Mic, Volume2, Radio } from 'lucide-react'
...
export function AccountTab() {
  ...
  const [micDevices, setMicDevices] = useState<DropdownOption[]>([
    { value: '', label: t('profile.audioDeviceDefault', 'Systemstandard') },
  ])
  const [selectedMicId, setSelectedMicId] = useState<string>(() => {
    try {
      return localStorage.getItem('msm_preferred_mic_id') || ''
    } catch {
      return ''
    }
  })
  ...
  const [audioNoiseSuppression, setAudioNoiseSuppression] = useState<boolean>(() => {
    try {
      return localStorage.getItem('msm_audio_noise_suppression') !== 'false'
    } catch {
      return true
    }
  })
  const [audioEchoCancellation, setAudioEchoCancellation] = useState<boolean>(() => {
    try {
      return localStorage.getItem('msm_audio_echo_cancellation') !== 'false'
    } catch {
      return true
    }
  })
  const [audioAutoGain, setAudioAutoGain] = useState<boolean>(() => {
    try {
      return localStorage.getItem('msm_audio_auto_gain') !== 'false'
    } catch {
      return true
    }
  })
  ...
  const handleSelectMic = (deviceId: string) => {
    ...
    try {
      localStorage.setItem('msm_preferred_mic_id', deviceId)
    } catch {}
    if (isTestingMic) {
      stopMicTest()
    }
  ...
  const startMicTest = async () => {
    ...
    try {
      ...
      try {
        ...
        if (selectedMicId) {
          // Fallback ohne exact deviceId wenn Gerät nicht mehr verfügbar
      ...
      setAudioTestError('Mikrofon konnte nicht gestartet werden. Bitte Berechtigungen prüfen.')
      setIsTestingMic(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Mikrofon & Audio */}
      <section className="msm-card p-6" aria-labelledby="audio-settings-title">
        <div className="flex items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-2">
            <Mic className="h-5 w-5 text-secondary" aria-hidden="true" />
            <h2 id="audio-settings-title" className="font-headline text-lg font-semibold text-on-surface">
          ...
          <span className={`inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-xs font-medium ${
            ...
            {isTestingMic ? t('profile.audioActive', 'Aufnahme aktiv') : t('profile.audioInactive', 'Nicht aktiv')}
          </span>
        </div>

        <p className="max-w-2xl font-body-md text-sm leading-6 text-on-surface-variant mb-5">
          {t('profile.audioDescription', 'Wähle dein bevorzugtes Mikrofon für Sprachnachrichten und Sprachübertragungen. Teste das Mikrofon mit Live-Pegelanzeige.')}
        </p>

        <div className="max-w-xl space-y-5">
          <div>
            <label
              htmlFor="profile-mic-dropdown"
              className="block font-label-md text-label-md text-on-surface-variant mb-1.5 uppercase tracking-wider"
            >
              {t('profile.audioDeviceLabel', 'Mikrofon-Eingabegerät')}
            </label>
            <Dropdown
              id="profile-mic-dropdown"
              value={selectedMicId}
              onChange={handleSelectMic}
              options={micDevices}
              aria-label={t('profile.audioDeviceLabel', 'Mikrofon-Eingabegerät')}
            />
          </div>

          {/* Audio Filters: Noise suppression, echo cancellation, auto gain */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
            <label className="flex items-center gap-2 text-xs text-on-surface cursor-pointer select-none">
              <input
                ...
                onChange={(e) => {
                  ...
                  setAudioNoiseSuppression(val)
                  try {
                    localStorage.setItem('msm_audio_noise_suppression', String(val))
                  } catch {}
                  if (isTestingMic) stopMicTest()
                }}
                className="rounded border-outline-variant text-primary focus:ring-primary"
              />
              <span>{t('profile.audioNoiseSuppression', 'Rauschunterdrückung')}</span>
            </label>
            ...
            <label className="flex items-center gap-2 text-xs text-on-surface cursor-pointer select-none">
              <input
                ...
                onChange={(e) => {
                  const val = e.target.checked
                  setAudioEchoCancellation(val)
                  try {
                    localStorage.setItem('msm_audio_echo_cancellation', String(val))
                  } catch {}
                  if (isTestingMic) stopMicTest()
                }}
                className="rounded border-outline-variant text-primary focus:ring-primary"
              />
              <span>{t('profile.audioEchoCancellation', 'Echounterdrückung')}</span>
            </label>

            <label className="flex items-center gap-2 text-xs text-on-surface cursor-pointer select-none">
              <input
                type="checkbox"
                checked={audioAutoGain}
                onChange={(e) => {
                  const val = e.target.checked
                  setAudioAutoGain(val)
                  try {
                    localStorage.setItem('msm_audio_auto_gain', String(val))
                  } catch {}
                  if (isTestingMic) stopMicTest()
                }}
                className="rounded border-outline-variant text-primary focus:ring-primary"
              />
              <span>{t('profile.audioAutoGain', 'Auto-Pegel (AGC)')}</span>
            </label>
          </div>

          {/* Live Level Meter if testing */}
          {isTestingMic && (
            <div className="p-3.5 rounded-xl border border-primary/30 bg-primary/5 space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-1.5 text-on-surface font-medium">
                  ...
                  <span>{t('profile.audioLevel', 'Pegel')}: {Math.round(micLevel * 100)}%</span>
                </span>
                <span className={micLevel > 0.05 ? 'text-emerald-400 font-semibold text-[11px]' : 'text-on-surface-variant/60 text-[11px]'}>
                  {micLevel > 0.05 ? t('profile.audioSignalDetected', 'Signal erkannt') : 'Kein Signal'}
                </span>
              </div>
              <div className="w-full h-3 rounded-full bg-surface-container-highest overflow-hidden p-0.5">
                <div
                  className="h-full rounded-full transition-all duration-75 ease-out"
                  style={{
                    width: `${Math.min(100, Math.round(micLevel * 100))}%`,
                    backgroundColor: micLevel > 0.75 ? '#f59e0b' : micLevel > 0.05 ? '#10b981' : '#6b7280',
                  }}
                />
              </div>
            </div>
          ...
          <div>
            <Button
              type="button"
              variant={isTestingMic ? 'secondary' : 'primary'}
              onClick={() => {
                if (isTestingMic) {
                  stopMicTest()
                } else {
            ...
            >
              ...
              <span>{isTestingMic ? t('profile.audioTestStop', 'Test beenden') : t('profile.audioTestStart', 'Mikrofon testen')}</span>
            </Button>
          </div>
        </div>
      </section>
