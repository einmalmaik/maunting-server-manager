    ...
    <div className="fixed inset-0 z-[70] flex flex-col bg-surface">
      <div className="shrink-0 px-3 py-2.5 border-b border-outline-variant/20 flex items-center gap-2">
        <button
          ...
          aria-label={t('messenger.cancelForward')}
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="min-w-0">
          <div className="text-sm font-semibold text-on-surface truncate">{t('messenger.forward')}</div>
          <div className="text-label-sm text-on-surface-variant">
            {t('messenger.forwardCount', { count: anzahlNachrichten })}
          </div>
        </div>
      </div>
      ...
      <div className="shrink-0 px-3 py-2">
        <Input
          ...
          placeholder={t('messenger.searchChatOrGroup')}
          prefix={<Search className="w-4 h-4" />}
          aria-label={t('messenger.searchTarget')}
        />
      </div>
        ...
        {fortschritt && fortschritt.gesamt > 0 && (
          <div className="space-y-1">
            <div className="flex justify-between text-label-sm text-on-surface-variant">
              <span>{t('messenger.reencrypting')}</span>
        ...
        >
          ...
          <span>
            {laeuft
              ? t('messenger.sending')
              : gewaehlt.length === 0
                ? t('messenger.chooseTarget')
                : t('messenger.sendToChats', { count: gewaehlt.length })}
          </span>
        </Button>
      </div>
    </div>,
