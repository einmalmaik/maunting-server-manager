    ...
    <div className="fixed inset-0 z-[70] flex flex-col bg-surface">
        ...
        <div className="min-w-0">
          ...
          {!gesperrt && !laeuft && (
            <div className="text-[11px] text-on-surface-variant tabular-nums">
              {gesamt === 0 ? 'Nichts gefunden' : `${gesamt} in ${chats.length} ${chats.length === 1 ? 'Chat' : 'Chats'}`}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto pb-[env(safe-area-inset-bottom)]">
        {gesperrt ? (
          ...
          <p className="py-16 text-center text-xs text-on-surface-variant/70">Wird durchgesehen …</p>
        ) : chats.length === 0 ? (
          ...
          chats.map((chat) => {
            ...
            return (
              <div key={chat.blindMailboxId} className="py-1">
                <div className="px-3 py-1.5 flex items-center gap-2 text-[11px] font-semibold text-on-surface-variant uppercase tracking-wide">
                  {meta?.isGroup ? (
                    <Users className="w-3.5 h-3.5 text-tertiary shrink-0" />
                  ) : (
                    <MessageSquare className="w-3.5 h-3.5 text-primary shrink-0" />
                  )}
                  <span className="truncate">{meta?.name || 'Unbekannter Chat'}</span>
                </div>
                {chat.treffer.map((t) => (
                  <button
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1 space-y-0.5">
                      <span className="flex items-baseline justify-between gap-2">
                        ...
                        <span className="text-[10px] text-on-surface-variant/70 shrink-0 tabular-nums">
