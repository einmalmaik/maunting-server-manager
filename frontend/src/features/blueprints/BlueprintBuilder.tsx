      ...
      <div className="space-y-5">
        ...
        {draft.mods.supportsCurseForge && (
          <div className="grid gap-4 sm:grid-cols-3">
            <Field id="bp-cf-game-id" label={t('blueprintBuilder.fields.curseforgeGameId.label')} help={t('blueprintBuilder.fields.curseforgeGameId.help')} error={issueFor('mods.curseforgeGameId')}><input className="msm-input font-mono" value={draft.mods.curseforgeGameId ?? ''} onChange={event => setDraft({ ...draft, mods: { ...draft.mods!, curseforgeGameId: event.target.value } })} /></Field>
            <Field id="bp-cf-class-id" label={t('blueprintBuilder.fields.curseforgeClassId.label')} help={t('blueprintBuilder.fields.curseforgeClassId.help')} error={issueFor('mods.curseforgeClassId')}><input className="msm-input font-mono" value={draft.mods.curseforgeClassId ?? ''} onChange={event => setDraft({ ...draft, mods: { ...draft.mods!, curseforgeClassId: event.target.value } })} /></Field>
            <Field id="bp-cf-install-path" label={t('blueprintBuilder.fields.curseforgeInstallPath.label')} help={t('blueprintBuilder.fields.curseforgeInstallPath.help')} error={issueFor('mods.curseforgeInstallPath')}><input className="msm-input font-mono" value={draft.mods.curseforgeInstallPath ?? ''} onChange={event => setDraft({ ...draft, mods: { ...draft.mods!, curseforgeInstallPath: event.target.value } })} /></Field>
          </div>
        )}
        ...
        {draft.mods.modInjection === 'startupArg' && (
          <div className="grid gap-4 sm:grid-cols-[1fr_160px]">
            <Field id="bp-mod-format" label={t('blueprintBuilder.fields.modFormat.label')} help={t('blueprintBuilder.fields.modFormat.help')} error={issueFor('mods.modStartupArgumentFormat')}><input className="msm-input font-mono" value={draft.mods.modStartupArgumentFormat ?? ''} onChange={event => setDraft({ ...draft, mods: { ...draft.mods!, modStartupArgumentFormat: event.target.value } })} /></Field>
            <Field id="bp-mod-sep" label={t('blueprintBuilder.fields.modSeparator.label')} help={t('blueprintBuilder.fields.modSeparator.help')}><input className="msm-input font-mono" value={draft.mods.modStartupArgumentSeparator ?? ';'} onChange={event => setDraft({ ...draft, mods: { ...draft.mods!, modStartupArgumentSeparator: event.target.value } })} /></Field>
          </div>
        )}
          ...
          <div className="grid min-h-0 min-w-0 flex-1 grid-rows-[auto_minmax(0,1fr)] overflow-hidden lg:grid-cols-[14rem_minmax(0,1fr)] lg:grid-rows-1">
            <nav className="min-w-0 overflow-x-auto border-b border-outline-variant/50 p-2 lg:overflow-y-auto lg:border-b-0 lg:border-r lg:p-3" aria-label={t('blueprintBuilder.sectionNavigation')}>
              <ol className="grid w-full grid-cols-4 gap-1 lg:grid-cols-1">
                {sectionIds.map((item, index) => <li key={item} className="min-w-0"><button type="button" onClick={() => setSection(item)} aria-current={section === item ? 'step' : undefined} className={`flex min-h-11 min-w-0 w-full flex-col items-center justify-center gap-0.5 rounded-lg px-1 text-center text-label-sm leading-tight lg:flex-row lg:justify-start lg:gap-3 lg:px-3 lg:text-left lg:text-sm ${section === item ? 'bg-primary/10 text-primary ring-1 ring-primary/20' : 'text-on-surface-variant hover:bg-surface-container-high'}`}><span className="font-mono text-label-sm opacity-60 lg:text-xs">{String(index + 1).padStart(2, '0')}</span><span className="min-w-0 break-words">{sectionLabel(item)}</span></button></li>)}
              </ol>
            </nav>
