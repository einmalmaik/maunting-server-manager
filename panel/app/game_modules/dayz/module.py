"""DayZ Linux Native game module implementation."""
...
from app.shell import PanelCommandError, run_game_command
from app.game_modules import GameManifest, GameModule, PortConfig, ServerStatus, TaskResult, register_module
...
def _manager_path() -> str:
    return get_settings().dayz_manager_path
...
class DayZModule:
    ...
    def install(self, server_dir: Path, server_name: str) -> TaskResult:
        try:
            run_game_command(_manager_path(), "install", server_name=server_name, expect_json=False)
    ...
    def update(self, server_dir: Path, server_name: str) -> TaskResult:
        try:
            run_game_command(_manager_path(), "update", server_name=server_name, expect_json=False)
    ...
    def validate(self, server_dir: Path, server_name: str) -> TaskResult:
        try:
            run_game_command(_manager_path(), "validate", server_name=server_name, expect_json=False)
    ...
    def start(self, server_dir: Path, server_name: str) -> TaskResult:
        try:
            run_game_command(_manager_path(), "start", server_name=server_name, expect_json=False)
    ...
    def stop(self, server_dir: Path, server_name: str) -> TaskResult:
        try:
            run_game_command(_manager_path(), "stop", server_name=server_name, expect_json=False)
    ...
    def restart(self, server_dir: Path, server_name: str) -> TaskResult:
        try:
            run_game_command(_manager_path(), "restart", server_name=server_name, expect_json=False)
    ...
    def get_status(self, server_dir: Path, server_name: str) -> ServerStatus:
        try:
            ...
            return ServerStatus(
                ...
                ports=[
                    PortConfig(name="game", port=data.get("port", 2302), protocol="udp"),
                    PortConfig(name="query", port=data.get("queryport", 27016), protocol="udp"),
                    PortConfig(name="rcon", port=data.get("rconport", 2311), protocol="tcp"),
                ],
            )
        except Exception:
    ...
    def wipe(self, server_dir: Path, server_name: str, scope: str) -> TaskResult:
        try:
            run_game_command(_manager_path(), "wipe", scope, server_name=server_name, expect_json=False)
    ...
    def backup(self, server_dir: Path, server_name: str) -> TaskResult:
        try:
            run_game_command(_manager_path(), "backup", "create", server_name=server_name, expect_json=False)
    ...
    def restore(self, server_dir: Path, server_name: str, backup_name: str) -> TaskResult:
        try:
            run_game_command(
                _manager_path(), "backup", "restore", backup_name,
                server_name=server_name, expect_json=False
            )
            return TaskResult(ok=True, message="Restore started.")
        except PanelCommandError as exc:
            return TaskResult(ok=False, message=str(exc))

    # -- Mods (DayZ uses Steam Workshop via steamcmd) --
